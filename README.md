Vertcoin Rosetta API implementation
====================================
...
<p align="center"><b>
ROSETTA-VERTCOIN IS UNDER INITIAL DEVELOPMENT AND IF IT IS NOT BROKEN, THIS IS ACCIDENTAL.
DO NOT USE THIS SOFTWARE, YET.
</b></p>

## Overview
`rosetta-vertcoin` provides an implementation of the Rosetta API for
Vertcoin in golang, based off the [rosetta-bitcoin](https://github.com/coinbase/rosetta-bitcoin)
...
### Install
Running the following commands will create a Docker image called `rosetta-vertcoin:latest`.

#### From GitHub
To download the pre-built Docker image from the latest release, run:
```text
curl -sSfL https://raw.githubusercontent.com/rosetta-vertcoin/rosetta-vertcoin/master/install.sh | sh -s
```
_Do not try to install rosetta-vertcoin using GitHub Packages!_
...
#### Mainnet:Online
```text
docker run -d --rm --ulimit "nofile=100000:100000" -v "$(pwd)/bitcoin-data:/data" -e "MODE=ONLINE" -e "NETWORK=MAINNET" -e "PORT=8080" -p 8080:8080 -p 5889:5889 rosetta-vertcoin:latest
```
_If you cloned the repository, you can run `make run-mainnet-online`._

#### Mainnet:Offline
```text
docker run -d --rm -e "MODE=OFFLINE" -e "NETWORK=MAINNET" -e "PORT=8081" -p 8081:8081 rosetta-vertcoin:latest
```
_If you cloned the repository, you can run `make run-mainnet-offline`._

#### Testnet:Online
```text
docker run -d --rm --ulimit "nofile=100000:100000" -v "$(pwd)/bitcoin-data:/data" -e "MODE=ONLINE" -e "NETWORK=TESTNET" -e "PORT=8080" -p 8080:8080 -p 15889:15889 rosetta-vertcoin:latest
```
_If you cloned the repository, you can run `make run-testnet-online`._

#### Testnet:Offline
```text
docker run -d --rm -e "MODE=OFFLINE" -e "NETWORK=TESTNET" -e "PORT=8081" -p 8081:8081 rosetta-vertcoin:latest
```
_If you cloned the repository, you can run `make run-testnet-offline`._

## System Requirements
`rosetta-vertcoin` has been tested on an [AWS c5.2xlarge instance](https://aws.amazon.com/ec2/instance-types/c5).
This instance type has 8 vCPU and 16 GB of RAM.

### Network Settings
To increase the load `rosetta-vertcoin` can handle, it is recommended to tune your OS
...
sysctl -p (when done)
```
_We have not tested `rosetta-vertcoin` with `net.ipv4.tcp_tw_recycle` and do not recommend
enabling it._
...
### Memory-Mapped Files
`rosetta-vertcoin` uses [memory-mapped files](https://en.wikipedia.org/wiki/Memory-mapped_file) to
persist data in the `indexer`. As a result, you **must** run `rosetta-vertcoin` on a 64-bit
...
## Architecture
`rosetta-vertcoin` uses the `syncer`, `storage`, `parser`, and `server` package
from [`rosetta-sdk-go`](https://github.com/coinbase/rosetta-sdk-go) instead
of a new Vertcoin-specific implementation of packages of similar functionality. Below
...
### Optimizations
* Automatically prune vertcoind while indexing blocks
...
To speed up indexing, `rosetta-vertcoin` uses concurrent block processing
with a "wait free" design (using channels instead of sleeps to signal
which threads are unblocked). This allows `rosetta-vertcoin` to fetch
...
To validate `rosetta-vertcoin`, [install `rosetta-cli`](https://github.com/rosetta-vertcoin/rosetta-cli#install)
