...
build:
	docker build -t rosetta-vertcoin:latest https://github.com/rosetta-vertcoin/rosetta-vertcoin.git

build-local:
	docker build -t rosetta-vertcoin:latest .

build-release:
	# make sure to always set version with vX.X.X
	docker build -t rosetta-vertcoin:$(version) .;
	docker save rosetta-vertcoin:$(version) | gzip > rosetta-vertcoin-$(version).tar.gz;

run-mainnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/bitcoin-data:/data" -e "MODE=ONLINE" -e "NETWORK=MAINNET" -e "PORT=8080" -p 8080:8080 -p 5889:5889 rosetta-vertcoin:latest

run-mainnet-offline:
	docker run -d --rm -e "MODE=OFFLINE" -e "NETWORK=MAINNET" -e "PORT=8081" -p 8081:8081 rosetta-vertcoin:latest

run-testnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/bitcoin-data:/data" -e "MODE=ONLINE" -e "NETWORK=TESTNET" -e "PORT=8080" -p 8080:8080 -p 15889:15889 rosetta-vertcoin:latest
