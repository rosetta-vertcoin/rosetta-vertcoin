...
usage() {
  this=$1
  cat <<EOF
$this: download pre-compiled Docker images for rosetta-vertcoin /rosetta-vertcoin
...
execute() {
  ...
  docker tag "rosetta-vertcoin:${TAG}" "rosetta-vertcoin:latest"
  log_info "loaded rosetta-vertcoin:${TAG} and tagged as rosetta-vertcoin:latest"
  rm -rf "${tmpdir}"
...
BINARY=rosetta-vertcoin
FORMAT=tar.gz
OWNER=rosetta-vertcoin
REPO="rosetta-vertcoin"
PREFIX="$OWNER/$REPO"
